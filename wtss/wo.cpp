...
implementation
   ...
   
  {
   ...
   while Pr == true do {
    ...
    if RetraceFile == '' then {
     ...
     Result  =  0;
     if Compare2Rands(RetraceRand,Rand) == false then {
    ...
    if Result == 0 then {
     ...
     if Index[B] == -1 then {
      ...
      while Next<>-1 do {
       PrS  =  true;
       for I  =  0 to 11 do {
        if SearchBuffer^[Next].SRes[I]<>SRes[I] then {
 	 PrS  =  false;
 	 break;
	};
       };
	...
	////Ищем пары KI////
	 ...
	 SC_Uk  =  0;
	  ...
	  R8_after_R1[I][1]  =  Comp128v1_T0[2*KiPair2[1]+I];
         ...
         for CurPair[0]  =  0 to $FF do {
          for CurPair[1]  =  0 to $FF do {
	   ...
	   PrS  =  false;
           if IndexCollBuffer^[EmulRes[0]][EmulRes[1]] == -1 then {
	    ...
	    PrS  =  false;
            while NextColl<>-1 do {
	     if (CollBuffer^[NextColl].B2 == EmulRes[2]) and (CollBuffer^[NextColl].B3 == EmulRes[3]) then {
	 ...
	 while (RetraceFile<>'') and (GetStringFromRetrace(RetraceRand,CSRes1)<>0) do ;
         if RetraceFile == '' then {
          ...
          Result  =  0;
          if Compare2Rands(RetraceRand,CRand1) == false then {
           LWriteln('ERROR! Invalid RAND in string '+IntToStr(RetraceStringCount)+' in retrace file!',2);
	 ...
	 while (RetraceFile<>'') and (GetStringFromRetrace(RetraceRand,CSRes2)<>0) do ;
         if RetraceFile == '' then {
          ...
          Result  =  0;
          if Compare2Rands(RetraceRand,CRand2) == false then {
           LWriteln('ERROR! Invalid RAND in string '+IntToStr(RetraceStringCount)+' in retrace file!',2);
   ...
   
  {
   ...
   for CurPair[0]  =  0 to $FF do {
    for CurPair[1]  =  0 to $FF do {
     ...
     for CalcByte  =  0 to 3 do {
      if (IndexBuffer^[Search[CalcByte].B0][Search[CalcByte].B1] == -1) then {
	...
	if (DataBuffer^[Next].B2 == Search[CalcByte].B2) and (DataBuffer^[Next].Nomer == CalcByte) and
	 (DataBuffer^[Next].BD<>Search[CalcByte].BD) then {
   ...
   if Res_Uk<>0 then {
    ...
    for Cur_Uk  =  0 to (Res_Uk-1) do {
     ...
     for I  =  0 to 62 do {
      if ((RandPair_3R_v1_Common[I].Pair[0] == ResBuffer^[Cur_Uk].Res[0][CalcByte]) and 
       ...
       (RandPair_3R_v1_Common[I].Pair[1] == ResBuffer^[Cur_Uk].Res[0][CalcByte])) then {
    ...
    PrStep  =  false;
    Cur_Uk  =  0;
    while Cur_Uk< == (Res_Uk-1) do {
     CalcByte  =  ResBuffer^[Cur_Uk].Nomer;
     if ((PrStep == false) and ((CalcByte == 1) or (CalcByte == 3))) or
      ((PrStep == true) and ((CalcByte == 0) or (CalcByte == 2))) then {
      ...
      for CollCounter  =  0 to $FF do {
       ...
       if RetraceFile == '' then {
        ...
        Result  =  0;
        if Compare2Rands(RetraceRand,Rand1) == false then {
         LWriteln('ERROR! Invalid RAND in string '+IntToStr(RetraceStringCount)+' in retrace file!',2);
       ...
       if RetraceFile == '' then {
        ...
        Result  =  0;
        if Compare2Rands(RetraceRand,Rand2) == false then {
         LWriteln('ERROR! Invalid RAND in string '+IntToStr(RetraceStringCount)+' in retrace file!',2);
	...
	LWrite('Checking collision for 3R...',2);
        ...
        if RetraceFile == '' then {
         ...
         Result  =  0;
         if Compare2Rands(RetraceRand,RandCheck1) == false then {
          LWriteln('ERROR! Invalid RAND in string '+IntToStr(RetraceStringCount)+' in retrace file!',2);
        ...
        if RetraceFile == '' then {
         ...
         Result  =  0;
         if Compare2Rands(RetraceRand,RandCheck2) == false then {
          LWriteln('ERROR! Invalid RAND in string '+IntToStr(RetraceStringCount)+' in retrace file!',2);
	...
	FoundPair2[1]  =  CollB[2];
        for CurCalcPair[0]  =  0 to $FF do {
         for CurCalcPair[1]  =  0 to $FF do {
          ...
          PrS  =  true;
          for I  =  0 to 7 do {
           if EmulKiRes1[I]<>EmulKiRes2[I] then {
            PrS  =  false;
            break;
           };
          };
	...
	if AllKiFound>0 then {
	 for I  =  1 to 16 do KiVarGood[I]  =  0;
	 PrCollGood  =  false;
	 for CurKiVar  =  1 to AllKiFound do {
 	  LWrite('Checking KI pair '+PrintHexPair(KiVar[CurKiVar],false)+'...',2);
	  PrUncol  =  false;
	  for I  =  0 to 768 do {
	   if (KiVar[CurKiVar,0] == UncolPair_2R_v1[I,0]) and (KiVar[CurKiVar,1] == UncolPair_2R_v1[I,1]) then {
 	    PrUncol  =  true;
	    break;
	   };
	   if (UncolPair_2R_v1[I,0]>KiVar[CurKiVar,0]) or 
	    ((UncolPair_2R_v1[I,0] == KiVar[CurKiVar,0]) and (UncolPair_2R_v1[I,1]>KiVar[CurKiVar,1])) then {
 	    break; 
	   }; 
	  };
	  if PrUncol == true then {
     ...
     if Pr3RAttack == true then break;
     Cur_Uk  =  Cur_Uk+1;
     if Cur_Uk>(Res_Uk-1) then {
      if PrStep == false then {
       PrStep  =  true;
       Cur_Uk  =  0;
      };
     };
 ...
 function A4Rv1(var Port:TBlockSerial;Pair:byte;CPair1,CPair2:TPair;var KIPair:TPair):Longint;
      ...
      
  {
   ...
   Res_Uk  =  0;
   for P1  =  0 to (Res1_Uk-1) do {
    for P2  =  0 to (Res2_Uk-1) do {
     if (ResBuffer1^[P1].Nomer == ResBuffer2^[P2].Nomer) then {
      ...
      for I  =  0 to 2 do {
       for J  =  (I+1) to 3 do {
        if (Concat8[I][0] == Concat8[J][0]) and (Concat8[I][1]<>Concat8[J][1]) then {
	 ...
	 for K  =  0 to 13 do {
          if ((RandPair_4R_v1_Common[K].Pair[0] == Concat8[I][1]) and 
           ...
           (RandPair_4R_v1_Common[K].Pair[1] == Concat8[I][1])) then {
           AllFound  =  1;
	   CommonUk  =  K;
           break;
          };
   ...
   while Cur_Uk< == (Res_Uk-1) do {
    if ((PrStep == false) and (ResBuffer8^[Cur_Uk].CommonUk< == 7)) or
     ((PrStep == true) and (ResBuffer8^[Cur_Uk].CommonUk> == 8)) then {
	...
	ResR1_0[1]  =  Comp128v1_T0[2*CurPair[0]];
 	 ...
 	 if PrS then {
	  PrS  =  true;
	  for I  =  1 to AllKiFound do {
	   if (KiVar[I][0] == CurPair[0]) and (KiVar[I][1] == CurPair[1]) then {
	    PrS  =  false;
	    break;
	   };
	  };
 ...
 function A5Rv1(var Port:TBlockSerial;Pair:byte;CData:TData8;var KIPair:TPair):Longint;
   ...
   
  {
     ...
     if (ResBuffer1^[P1].Nomer == ResBuffer2^[P2].Nomer) then {
      ...
      if SearchPairArray[PN]<12 then {
       ...
       for I  =  0 to 2 do {
        for J  =  (I+1) to 3 do {
         if (Concat8[I][0] == Concat8[J][0]) and (Concat8[I][1]<>Concat8[J][1]) then {
 	  if (ResCheck[PN][Concat8[I][1]][Concat8[J][1]]) == 0 then {
   ...
   for P1  =  0 to (Res81_Uk-1) do {
    for P2  =  0 to (Res82_Uk-1) do {
     if (ResBuffer81^[P1].Nomer == ResBuffer82^[P2].Nomer) then {
      ...
      for I  =  0 to 2 do {
       for J  =  (I+1) to 3 do {
        if (Concat16[I][0] == Concat16[J][0]) and (Concat16[I][1]<>Concat16[J][1]) then {
	 ...
	 for K  =  0 to 5 do {
          if ((RandPair_5R_v1_Common[K].Pair[0] == Concat16[I][1]) and 
           ...
           (RandPair_5R_v1_Common[K].Pair[1] == Concat16[I][1])) then {
           AllFound  =  1;
	   CommonUk  =  K;
           break;
          };
         ...
         if AllFound == 1 then {
	  if (ResCheck16[ResBuffer81^[P1].Nomer][Concat16[I][1]][Concat16[J][1]]) == 0 then {
   ...
   while Cur_Uk< == (Res_Uk-1) do {
    if ((PrStep == false) and (ResBuffer^[Cur_Uk].CommonUk< == 2)) or
     ((PrStep == true) and (ResBuffer^[Cur_Uk].CommonUk> == 3)) then {
     LWriteln('Using two data groups:',2);
...
@2:
 ...
 function BForce2Pv1(var Port:TBlockSerial;Pairs:byte;KI:TData16;var KIPair1,KIPair2:TPair):Longint; 
   ...
   STemp:string;
  {
   ...
   AllKiFound  =  0;
   LWrite('000/255',1);
   for CurPair1[0]  =  0 to 255 do {
    KI[Pair1]  =  CurPair1[0];
    for CurPair1[1]  =  0 to 255 do {
     KI[Pair1+8]  =  CurPair1[1];
     for CurPair2[0]  =  0 to 255 do {
      KI[Pair2]  =  CurPair2[0];
      for CurPair2[1]  =  0 to 255 do {
 ...
 function ChKIv1(var Port:TBlockSerial;KI:TData16):Longint; 
   ...
   
  {
   ...
   for Pair  =  0 to 7 do {
    ...
    PrUncol  =  false;
    for I  =  0 to 768 do {
     if (KiPair[0] == UncolPair_2R_v1[I,0]) and (KiPair[1] == UncolPair_2R_v1[I,1]) then { 
      PrUncol  =  true;
      break;
     };
     if (UncolPair_2R_v1[I,0]>KiPair[0]) or 
      ((UncolPair_2R_v1[I,0] == KiPair[0]) and (UncolPair_2R_v1[I,1]>KiPair[1])) then {
      break; 
     }; 
    };
    if PrUncol == true then {
