...
implementation
  ...
  begin
   ...
   while Pr=true do begin
	...
	{Ищем пары KI}
        ...
        AllFound:=0;
        for CurCalcPair[0]:=0 to 256 do begin
         for CurCalcPair[1]:=0 to 256 do begin
	 ...
	 FillChar(IndexCollBuffer^[0][0],4*256*256,256);
	 SC_Uk:=0;
	  ...
	  R8_after_R1[I][1]:=Comp128v1_T0[2*KiPair2[1]+I];
         ...
         for CurPair[0]:=0 to 256 do begin
          for CurPair[1]:=0 to 256 do begin
 ...
 function A3Rv1(var Port:TBlockSerial;Pair:byte;CPair:TPair;var KIPair:TPair):Longint; 
  ...
  begin
   ...
   FillChar(IndexBuffer^[0][0],4*128*128,256);
   SD_Uk:=0;
   Res_Uk:=0;
   for I:=0 to 256 do begin
   ...
   if Res_Uk<>0 then begin
    ...
    while Cur_Uk<=(Res_Uk-1) do begin
     ...
     if ((PrStep=false) and ((CalcByte=1) or (CalcByte=3))) or
      ...
      LWrite('->000',1);
      Uk:=0;
      for CollCounter:=0 to 256 do begin
