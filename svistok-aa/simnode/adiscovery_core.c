...
defdev_t defdevs[] = {
	...
	{200, 1, 0x12d1, 0x1001, {
				    ...
				    {0,{0x00,0x00,0x00,0x04,0x00,0x00,0x00,0x00,0x00,0x00},{0}}
				}, 2, 1, -1
	},
...
//cdc_ether - 1550 + 2 ttyUSB
	...
	{9173, 1, 0x12d1, 0x1436, {
				    {1,{0x01,0x00,0x00,0x00,0x81,0x82,0x00,0x00,0x00,0x00},{0}},
				    {0,{0x00,0x00,0x00,0x00,0x00,0x00,0x83,0x00,0x00,0x00},{0}},
				    {0,{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},{0}},
...
static const char sys_drivers[DRCOUNT][256] = {"/sys/bus/usb/drivers/option","/sys/bus/usb/drivers/usbserial_generic","/sys/bus/usb/drivers/pl2303","/sys/bus/usb/drivers/cdc_ether"};
...
void sysdev_show(FILE *fd,sysdev_t * sysdev)
{
		...
		for(o=0;o<IFCOUNT;o++)
			if(sysdev->iface[o].enabled!=0)
				...
				alog(fd,">       iface=%d { %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %s } \n",o,
				sysdev->iface[o].ep[0],
				sysdev->iface[o].ep[1],
				sysdev->iface[o].ep[2],
				sysdev->iface[o].ep[3],
