...
static int __a_dsp_call_progress(struct ast_dsp *dsp, short *s, int len)
{
	int x;
	int y;
	int pass;
	...
	int res = 0;
	ast_debug(5,"dsp_call_progress len=%d dsp->gsamp_size %d -  dsp->gsamps %d\n", len,dsp->gsamp_size, dsp->gsamps);
		...
		if (dsp->gsamps == dsp->gsamp_size) {
			...
			for (y = 0; y < 7; y++) {
				...
				if (hz[HZ_425] > TONE_MIN_THRESH * TONE_THRESH)
				{
					...
					ast_verb(3,"DSP_TONE_STATE_RINGING\n");
					return AST_CONTROL_RINGING;
				} else if (dsp->genergy > TONE_MIN_THRESH * TONE_THRESH/2) {
					newstate = DSP_TONE_STATE_TALKING;
					ast_verb(3,"DSP_TONE_STATE_TALKING\n");
					return AST_CONTROL_ANSWER;
				} else {
					newstate = DSP_TONE_STATE_SILENCE;
				}

				if (newstate == dsp->tstate) { dsp->tcount++; }


				if (dsp->tstate==DSP_TONE_STATE_RINGING) 
				{
					if    (dsp->tcount >= 2)  //THRESH_RING
					{
						res = AST_CONTROL_RINGING;
					}
				}
				if (dsp->tstate==DSP_TONE_STATE_TALKING)
				{
					if    (dsp->tcount >= 2) //THRESH_TALK
					{
...
int a_dsp_call_progress(struct ast_dsp *dsp, struct ast_frame *inf)
{
	...
	return __a_dsp_call_progress(dsp, inf->data.ptr, inf->datalen / 2);
}
...
struct ast_frame *ast_dsp_process(struct ast_channel *chan, struct ast_dsp *dsp, struct ast_frame *af)
{
	...
	if ((dsp->features & DSP_FEATURE_CALL_PROGRESS)) {
		res = 0;//__a_dsp_call_progress(dsp, shortdata, len);
		if (res) {
			switch (res) {
